...
alias lunch.gtv-4.0-jb-mr0='cd $GOOGLETVDIR/gtv-4.0-jb-mr0; . build/envsetup.sh; lunch cosmo-eng; source.envsetup'
alias lunch.gtv-4.0-jb-mr1='cd $GOOGLETVDIR/gtv-4.0-jb-mr1; . build/envsetup.sh; lunch cosmo-eng; source.envsetup'
...
#alias makewithlog='time make -j8 2>&1 | tee make_$TARGET_PRODUCT-$TARGET_BUILD_VARIANT-`date +'%y.%m.%d_%H%M%S'`.log; ll $OUT/system.img | grep $OUT/system.img'
alias makewithlog='$MAKEWITHLOG_SH'
#alias makeotawithlog='time make otapackage -j8 2>&1 | tee make_$TARGET_PRODUCT-$TARGET_BUILD_VARIANT-otapackage_`date +'%y.%m.%d_%H%M%S'`.log; ll $OUT/$TARGET_PRODUCT-ota-$TARGET_BUILD_VARIANT.$USER.zip | grep $OUT/$TARGET_PRODUCT-ota-$TARGET_BUILD_VARIANT.$USER.zip'
alias makeotawithlog='$MAKEOTAWITHLOG_SH'
#alias gtv_reinstall='$ADBCON_SH && vendor/tv/tools/gtv_reinstall.sh -f $OUT/$TARGET_PRODUCT-ota-$TARGET_BUILD_VARIANT.$USER.zip $ADBHOST'
alias gtv_reinstall='$GTV_REINSTALL_SH'
#alias do_all_at_once='makewithlog && makeotawithlog && gtv_reinstall'
